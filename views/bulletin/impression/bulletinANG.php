...
foreach ($rangs as $rang) {
    ...
    if ($codeperiode === "A") {
        ...
        $titre = '<div>MONTHLY PROGRESS REPORT CARD N° ' . ($sequence['LIBELLEHTML'][0]).'</div>';
    }elseif ($codeperiode === "T"){
...
# Le cadre pour la photo
    ...
    $pdf->WriteHTMLCell(0, 5, 56, $y, 'Reg. Number');
    $pdf->WriteHTMLCell(0, 5, 88, $y, ':&nbsp;<b>' . $rang['MATRICULEEL'] . '</b>');

    $pdf->WriteHTMLCell(0, 5, 56, $y + 4, 'First and Last name');
    $pdf->WriteHTMLCell(0, 5, 88, $y + 4, ':&nbsp;<b>' . $rang['NOMEL'] . " " . $rang['PRENOMEL'] . ' ' . $rang['AUTRENOMEL'] . '</b>');
    $pdf->WriteHTMLCell(0, 5, 56, $y + 8, 'Date and place of birth ');
    $naiss =   ':&nbsp;<b>' . $d->getDate() . " " . $d->getMois(3) . "-" . $d->getYear();
    if (!empty($rang['LIEUNAISSEL'])) {
        $naiss .= " At " . $rang['LIEUNAISSEL'];
    }
    $naiss .= '</b>';
    $pdf->WriteHTMLCell(0, 5, 88, $y + 8, $naiss);
    ...
    if ($codeperiode === "S") {
        ...
        $corps = printTravailANG($travail, $disc, $rang);
    } elseif ($codeperiode === "T") {
...
# Numero de la page
    ...
    if($codeperiode === "A"){
        ...
        $pdf->SetX(140);
        $finaldecision = getAnnuelFinalDecisionANG($rang['MOYGENERALE'], $classe['GROUPE']);
