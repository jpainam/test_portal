        ...
        for($jour = 1; $jour <= 6; $jour++){
            ...
            if(!isFusionner($ens, $array_fusionner)){
                if($ens != null){
                    ...
                    echo "<label class='el2'>" . $ens['CIVILITE']. ' '. $ens['NOM'] . '<br/>' . $ens['PRENOM'] . "</label></span></td>";
                }else{
                    echo "<td></td>";
                }
            }
...
<!-- div style="margin: 10px;text-align: center" id="btn_sync_emplois">
    <input style="width: 350px; border: 2px outset buttonface; margin:0" type="button" 
           value="<?php //echo __t("Synchroniser l'emploi du temps");?> (<?php //echo $nb_sync_emplois; ?>)" onclick="synchroniserEmploisDuTemps()"/>
</div -->
